...
const Sidebar = ({ activeTab, setActiveTab, isOpen, onClose }: { activeTab: string, setActiveTab: (tab: string) => void, isOpen: boolean, onClose: () => void }) => {
  ...
  return (
    <>
      ...
      <div className={`h-full w-64 glass-panel-heavy fixed left-0 top-0 flex flex-col z-50 transition-transform duration-300 transform md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
